...
router.get("/", auth, async function(req, res){
    try {
        const posts = await Post.find().sort({ date: -1 });  // sort from newest to oldest
        res.json(posts);
    } catch (err) {
        console.error(err.message);
        res.status(500).send("Server Error");
...
router.delete("/:id", auth, async function(req, res){
    try {
        ...
        // check if the post is of the user who is deleting
...
router.put('/like/:id', auth, async function(req, res){
    try {
        ...
        // using filter - higher order array method
...
router.put('/unlike/:id', auth, async function(req, res){
    try {
        ...
        const removeIndex = post.likes.map(function(like){
...
router.delete("/comment/:id/:comment_id", auth, async function(req,res){
    try {
        ...
        //Get remove index
        const removeIndex = post.comments.map(function(comment){
